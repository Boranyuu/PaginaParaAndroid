...
import { collection, addDoc, updateDoc, deleteDoc, doc, getDocs, query, where, getCountFromServer } 
...
export function abrirModalParaEditarCita(cita) {
  ...
  if (citaSeleccionada.actividadId) {
    ...
    actBeneficiarios.textContent = Array.isArray(citaSeleccionada.beneficiarios) ? citaSeleccionada.beneficiarios.join(", ") : (citaSeleccionada.beneficiarios ?? "");
...
export function llenarDatosActividad(actividadId) {
  ...
  actOferente.textContent = act.oferenteNombre ?? "";
  actBeneficiarios.textContent = Array.isArray(act.beneficiarios) ? act.beneficiarios.join(", ") : (act.beneficiarios ?? "");
...
export async function puedeReservar(actividadId, fechaISO) {
  ...
  try {
    const q = query(collection(db, CITAS_COL), where("actividadId", "==", actividadId), where("fechaHora", "==", fechaISO));
    let count = 0;
    try {
...
export async function guardarCita(fechaISO) {
  ...
  const citaObj = {
    ...
    oferente: actividad.oferenteNombre ?? null,
...
export async function reagendarCita(id, nuevaISO, actividadId) {
  ...
  await updateDoc(doc(db, CITAS_COL, id), { fechaHora: nuevaISO, fechaInicioMillis: Date.parse(nuevaISO), fechaModificacion: Date.now() });
}
...
btnGuardar.addEventListener("click", async () => {
  ...
  // Recargar calendario si está definido globalmente
